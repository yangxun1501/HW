...
class PandasChain:
    ...
    def __commit_block(self,block): 
        ...
        # Create block hash
        #the previous block's hash, the chains hash id, current date time, sequence id of the block and the root Merkle hash.
        block_hash = hashlib.sha256(str(str(self.__prev_hash)+str(self.__id)+str(dt.datetime.now())+str(self.__seq_id)+str(self.__current_block.get_simple_merkle_root())).encode('utf-8')).hexdigest()
        # Add code here
...
class Block:
    ...
    def display_header(self): 
        print(str(self.__seq_id) + ", " + self.__status + ", " + str(self.__block_hash) + ", " + str(self.__prev_hash) + ", " + str(self.__merkle_tx_hash) + ", "+str(self.get_size())  )
