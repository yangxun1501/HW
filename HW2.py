...
def show_menu(menu):
    print("The Menu include following: ")
    for i in range(len(menu)):
        print(i+1, '. ', menu[i])

def buy_item(menu, user_input):
...
def display_menu(menu):
    ...
    show_menu(menu)
    user_input = input("Please select an item by item number (enter 'e' to exit): ")
    # if user exit without buy or sell, 0 will be returned
    last_valid_user_input = '0'
    while user_input != 'e':
        if user_input in menu_list:
            buy_item(menu, int(user_input))
            last_valid_user_input = user_input
        else:
            print("The input entered is not valid, please try again.")
        show_menu(menu)
        user_input = input("Please select an item by item number (enter 'e' to exit): ")
    
    print("The last valid user input (menu number) before exit: " + last_valid_user_input)
    print("The length of the menu excluding exit is " + str(menu_n))
    return int(last_valid_user_input), menu_n
